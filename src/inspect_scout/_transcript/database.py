...
from .s3_log_cache import get_cached_df, put_cached_df, resolve_logs_with_etag
            ...
            if s3_logs:
                with inspect_kvstore("scout_s3_log_cache") as kvstore:
                    for log_path, etag in s3_logs:
                        # Try cache first
                        cached_df = get_cached_df(kvstore, log_path, etag)
                        ...
                        if cached_df is not None:
                            ...
                            # Update cache
                            put_cached_df(kvstore, log_path, etag, fresh_df)
